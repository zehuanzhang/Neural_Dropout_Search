...
#svhn
def i_to_mutuablelayer_svhn(i):
    i_dict = {15: 1, 19: 2, 24: 3, 28: 4}
    return i_dict[i]

#cifar
def ij_to_mutuablelayer(i, j):
    ij_dict = {(5, 0): 1, (5, 1): 2, (6, 0): 3, (6, 1): 4}
    return ij_dict[(i,j)]


def index_to_bool_list(index, size):
...
def evaluate(output, input, target, model, args):
    with torch.no_grad():
        ...
        pred = pred.t()
        _ece = ece(output, target)#output:(9984,10) target:(9984,)
        _entropy = entropy(output)
        _error = error(pred, target)#pred:(1, 9984) target:(9984,)
