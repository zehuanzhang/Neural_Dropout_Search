...
class LayerChoice(Mutable):
    ...
    # def forward(self, set_mask, *args, **kwargs):
        """
        Returns
        -------
        tuple of tensors
        ...
        """
        # print('mutables.py 230')
        if isinstance(args[0], list):
            assert isinstance(args[0], list), 'set_mask must be list type'
            out, mask = self.mutator.on_forward_layer_choice_set_mask(self, args[0], args[1])
        else:
            out, mask = self.mutator.on_forward_layer_choice(self, *args, **kwargs)

        # out, mask = self.mutator.on_forward_layer_choice(self, *args, **kwargs)
        # print('out, mask:', out.shape, mask)
        if self.return_mask:
            return out, mask
        return out
