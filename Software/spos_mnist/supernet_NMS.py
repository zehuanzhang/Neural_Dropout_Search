...
if __name__ == "__main__":
    ...
    #
    parser.add_argument('--model', type=str, default='lenet_all',
                        help='the model that we want to train')
    parser.add_argument('--p', type=float,
                        default=0.375, help='dropout probability')
    ...
    if evolution:
        ...
        P = 16  # The size of population in each generation
        N = 10 # How many generations of population to be searched
        r = 0.5#0.25  # The ratio of networks that are used as parents for next generation
        params = {
            'mutate_prob': 0.9,  # The probability of mutation in evolutionary search
            'mutation_ratio': 0.5,
            'population_size': P,
            'max_time_budget': N,
            'parent_ratio': r,
    ...
    if load:
        ...
        # Search Result
        # print('Search Result:')
        error, ece, _ = classification_evaluation_loader(choice_dict, model, args, loader=test_loader)
        ...
        print('search_result:', search_result['accuracy'], search_result['ece'], search_result['entropy'], search_result['choice_dict'])
        ...
        # All Results
        # print('All Results')
        num_choice_layer_tuple = (4, 4, 2)
        choice_set_list = [
            {'choice_1': i, 'choice_2': j, 'choice_3': k}
        ...
        for i in tqdm(range(len(choice_set_list))):
            ...
            torch.backends.cudnn.deterministic = True
            #
            error, ece, _ = classification_evaluation_loader(choice_dict, model, args, loader=valid_loader)
            _, _, entropy = classification_evaluation_loader(choice_dict, model, args, loader=fake_loader)
