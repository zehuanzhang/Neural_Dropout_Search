...
class ArchManager:
    def __init__(self):
        self.num_choice_layer_tuple = (4, 4, 2)
...
class EvolutionFinder:
    ...
    def run_evolution_search(self, verbose=False):
        ...
        for i in tqdm(range(population_size)):
            ...
            results_pred = self.accuracy_predictor(model=self.model, choice_dict=child_pool[i], loader=self.valid_loader,
                                               args=self.args)
            ...
            accs.append(# - weight_of_ece * ece_pred
                        # self.accuracy_predictor(model=self.model, choice_dict=child_pool[i], valid_loader=self.valid_loader,
                        #                         loss=self.loss, metrics=self.metrics, device=self.device, num_samples=self.num_samples)
                         acc_pred
        ...
        for iter in tqdm(range(max_time_budget)):
            ...
            for i in range(population_size):
                ...
                acc_pred = 100 - results_pred[0]
                ece_pred = results_pred[1]
                _, _, aPE_pred = self.accuracy_predictor(model=self.model, choice_dict=child_pool[i],loader=self.fake_loader,
                                                         args=self.args)
            ...
            # print('---------------------------------------------')
            if {'choice_1': 3, 'choice_2': 3, 'choice_3': 0} in child_pool:
                print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
