...
def classification_evaluation_loader(choice_dict, model, args, loader):
    ...
    error, ece, entropy, outputs, targets = _evaluate_and_record(choice_dict, model, results, args, loader=loader)

    return error, ece, entropy
...
def _evaluate_with_loader(choice_dict, loader, model, args):
    ...
    for i, (input, target) in enumerate(loader):
        ...
        for j in range(args.samples):
            ...
            # t_end = time.time()
            # print('classification_evaluation_loader 132:', t_end - t_start)
            samples.append(out)
...
def _evaluate_and_record(choice_dict, model, results, args, loader, train=False, valid=False, test=True):
    # train_loader, val_loader = get_train_loaders(args)
    # test_loader = get_test_loader(args)

    if train:
        error, ece, entropy, nll, _, _ = _evaluate_with_loader(loader, model, args)#(train_loader, model, args)
    ...
    if valid:
        ...
        error, ece, entropy, nll, outputs, targets = _evaluate_with_loader(choice_dict, loader, model, args)#(choice_dict, val_loader, model, args)
    ...
    if test:
        # error, ece, entropy, nll, outputs, targets = _evaluate_with_loader(choice_dict, test_loader, model, args)
        error, ece, entropy, nll, outputs, targets = _evaluate_with_loader(choice_dict, loader, model, args)
    ...
    return error, ece, entropy, outputs, targets
