...
class Masksembles1D(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = x * self.masks.unsqueeze(1)#  [16, 1, 500]*[16, 1, 500]
        x = torch.cat(torch.split(x, 1, dim=0), dim=1)#[1, 16, 500]=
        return x.squeeze(0).float()

        # # print(inputs.shape, self.masks[0].unsqueeze(1).shape)
        # x = inputs * self.masks[0].unsqueeze(1)  # [16, 1, 500]*[16, 1, 500]
        # # print(x.shape)
        # return x.squeeze(0).float()#[16, 500]
